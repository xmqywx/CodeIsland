...
const translations = {
  ...
  "faq.a11": { zh: "因为 MioIsland 未经 Apple 公证（Notarization），macOS 会阻止首次打开。解决方法：前往 系统设置 → 隐私与安全性 → 往下滚动，找到「"Mio Island"已被阻止打开」→ 点击「仍要打开」→ 输入密码确认。只需操作一次，之后可正常启动。如果用 Homebrew 安装（brew install xmqywx/codeisland/codeisland）则会自动处理，无需手动操作。", en: "MioIsland is not notarized by Apple, so macOS Gatekeeper blocks the first launch. Fix: Go to System Settings → Privacy & Security → scroll down, find 'Mio Island was blocked' → click 'Open Anyway' → enter your password. This is a one-time step. If you install via Homebrew (brew install xmqywx/codeisland/codeisland), this is handled automatically." },
