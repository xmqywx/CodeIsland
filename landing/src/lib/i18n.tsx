...
const translations = {
  ...
  "faq.a11": { zh: "因为 MioIsland 未经 Apple 公证（Notarization），macOS 会阻止首次打开。解决方法：右键（或 Control + 点击）Mio Island.app → 选择「打开」→ 在弹窗中点击「打开」即可。只需操作一次，之后可正常启动。或者在终端运行：xattr -dr com.apple.quarantine /Applications/Mio\\ Island.app", en: "MioIsland is not notarized by Apple, so macOS Gatekeeper blocks the first launch. Fix: Right-click (or Control-click) Mio Island.app → choose 'Open' → click 'Open' in the dialog. This is a one-time step. Alternatively, run in Terminal: xattr -dr com.apple.quarantine /Applications/Mio\\ Island.app" },
